...
if ($_POST) {
    ...
    if (empty($data['h1'])) {
        $data['h1'] = $data['title']; // Если H1 не указан, используем заголовок
    }
    ...
    if (!empty($data['slug'])) {
        $existing = $newsModel->getNewsBySlug($data['slug']);
        if ($existing) {
            $data['slug'] = $data['slug'] . '-' . time();
        }
    }

    // Если нет ошибок - создаем новость
    if (empty($errors)) {
        try {
            $news_id = $newsModel->createNews($data);
            $success = true;
            
            // Редирект с сообщением об успехе
            header('Location: /admin/news/edit.php?id=' . $news_id . '&success=created');
            exit;
        } catch (Exception $e) {
            $errors[] = 'Ошибка при создании новости: ' . $e->getMessage();
        }
    }
}
...
// Заполняем данные формы (из POST или значения по умолчанию)
...
<head>
    ...
    <title>Создать новость | Админ-панель</title>
...
<body>
                        ...
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="h1" name="h1" 
                                       ...
                                       placeholder="H1 для страницы (если отличается от заголовка)">
                                <div class="form-help">Заголовок H1 на странице статьи (если не указан, используется основной заголовок)</div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-help">URL адрес новости (генерируется автоматически из заголовка)</div>
                            </div>

                            <div class="form-group">
                                <label for="excerpt">Краткое описание</label>
                                <textarea id="excerpt" name="excerpt" rows="3" 
                                          placeholder="Краткое описание новости для превью"><?php echo htmlspecialchars($form_data['excerpt']); ?></textarea>
                                <div class="form-help">Отображается в списке новостей и для SEO</div>
                            </div>
                    ...
                    <div class="form-sidebar">
                        ...
                        <div class="sidebar-section">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-help">Если не указана, используется текущее время</div>
                            </div>
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('✅ Страница создания новости загружена');
        ...
        setTimeout(() => {
            showNotification('Редактор готов для создания новости! 📝 HTML режим | H1/H2/H3 кнопки', 'success');
        }, 1000);
    });
    ...
    console.log('✅ Скрипт создания новости загружен. HTML режим доступен!');
    </script>
</body>
</html>